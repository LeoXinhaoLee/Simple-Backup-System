...
int stat_chmod(int fd, mode_t mode) {
    int chmod_flag = fchmod(fd, mode);
...
int stat_mtime(int fd, time_t mtime) {
    ...
    int utime_flag = futimes(fd, time_input);
    if (utime_flag == -1) {
        return -3;
    }
...
int tar_write(const int fd, struct tar_t ** archive, const size_t filecount, const char * files[], const char verbosity,
    ...
    // write entries first
    if (write_entries(fd, tar, archive, filecount, files, &offset, verbosity, move) < 0){
...
int tar_extract(const int fd, struct tar_t * archive, const char verbosity, const char * prefix){
    // prefix: target path: ../src_dir
    int ret = 0;
...
int tar_update(const int fd, struct tar_t ** archive, const size_t filecount, const char * files[], const char verbosity){
    ...
    char ** newer = (char**) calloc(filecount, sizeof(char *));
    ...
    for(int i = 0; i < filecount; i++){
        ...
        newer[count] = (char*) calloc(strlen(files[i]) + 1, sizeof(char));
...
struct tar_t * exists(struct tar_t * archive, const char * filename, const char ori){
    // ori =
    // fileame is the name of the current file
    // if that name can also be found in previoused added archive
    // then the current file is a hard link
    while (archive){
        if (ori){
...
int format_tar_data(struct tar_t * entry, const char * filename, const char verbosity, int move){
    if (!entry){
    ...
    strncpy(entry -> original_name, filename, 100);    // original name: full path, only used for copying data into pack file
    strncpy(entry -> name, filename + move, 100);  // name: suffix, only start from source dir, used for unpacking relative hierarchey
    ...
    switch (st.st_mode & S_IFMT) {
        ...
        case S_IFLNK:
            ...
            // get link name: read the content of filename(path) to entry->link_name(buf)
...
int extract_entry(const int fd, struct tar_t * entry, const char verbosity, const char * pref){
    ...
    if ((entry -> type == REGULAR) || (entry -> type == NORMAL) || (entry -> type == CONTIGUOUS)){
        ...
        if (recursive_mkdir(full_path.c_str(), DEFAULT_DIR_MODE, verbosity) < 0){
        ...
        string entry_path = prefix + entry_name;
        int f = open(entry_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, oct2uint(entry -> mode, 7) & 0777);
        if (f < 0){
        ...
        if (mknod(full_path.c_str(), oct2uint(entry -> mode, 7), (oct2uint(entry -> major, 7) << 20) | oct2uint(entry -> minor, 7)) < 0){
            EXIST_ERROR("Unable to make device %s: %s", entry -> name, strerror(rc));
        }
    }
    else if (entry -> type == HARDLINK){

        string entry_name(entry->link_name);    // origianl file's relative path to current hard link
        string hard_link_name(entry->name);     // src_dir/a_hard.txt
        ...
        // remove file from path
        while (--len && (path[len] != '/'));
        path[len] = '/';   // if nothing was found, path is terminated
        path[len+1] = '\0';

        string original_file_prefix(path);

        string full_path = prefix + original_file_prefix + entry_name;
        string full_new_path = prefix + hard_link_name;    // ../src_dir/a_soft.txt

        // entry->link_name: name of original file
        // entry->name: name of the hard link
        /*
        if (link(entry -> link_name, entry -> name) < 0){
            EXIST_ERROR("Unable to create hardlink %s: %s", entry -> name, strerror(rc));
        }
         */
        if (link(full_path.c_str(), full_new_path.c_str()) < 0){    // link(old_path, new_path)
    ...
    else if (entry -> type == SYMLINK){     // extract symboli link

        string entry_name(entry->link_name);    // a.txt: should be src_dir/a_soft.txt
        string soft_link_name(entry->name);     // src_dir/a_soft.txt
        ...
        //string full_path = prefix + entry_name;          // ../a.txt: should be ../src_dir/a.txt
        string full_path = prefix + original_file_prefix + entry_name;
        string full_new_path = prefix + soft_link_name;    // ../src_dir/a_soft.txt

        /*
        if (symlink(full_path.c_str(), entry -> name) < 0){
            EXIST_ERROR("Unable to make symlink %s: %s", entry -> name, strerror(rc));
        }
         */
        if (symlink(full_path.c_str(), full_new_path.c_str()) < 0){
        ...
        if (mknod(full_path.c_str(), S_IFCHR | (oct2uint(entry -> mode, 7) & 0777), (oct2uint(entry -> major, 7) << 20) | oct2uint(entry -> minor, 7)) < 0){
        ...
        if (mknod(full_path.c_str(), S_IFBLK | (oct2uint(entry -> mode, 7) & 0777), (oct2uint(entry -> major, 7) << 20) | oct2uint(entry -> minor, 7)) < 0){
            EXIST_ERROR("Unable to create directory %s: %s", entry -> name, strerror(rc));
        }
    }
...
int write_entries(const int fd,
                  struct tar_t ** archive,
                  struct tar_t ** head,
                  const size_t filecount,
                  const char * files[],
                  int * offset, const char verbosity, int move){
    if (fd < 0){
        ERROR("Bad file descriptor");
    }
    ...
    for(unsigned int i = 0; i < filecount; i++){
        *tar = (tar_t*) malloc(sizeof(struct tar_t));
        ...
        // stat file
        if (format_tar_data(*tar, files[i], verbosity, move) < 0){
            WRITE_ERROR("Failed to stat %s", files[i]);
        }

        (*tar) -> begin = *offset;  //where the header of the current file begins in Archive
        ...
        if ((*tar) -> type == DIRECTORY){
             ...
             */
            const size_t len = strlen((*tar) -> original_name);
            char * parent = (char*) calloc(len + 1, sizeof(char));
            strncpy(parent, (*tar) -> original_name, len);

            // add a '/' character to the end, e.g., (*tar)->name: src_dir --> src_dir/
            ...
            while ((dir = readdir(d))){
                ...
                if (strncmp(dir -> d_name, ".", sublen) && strncmp(dir -> d_name, "..", sublen)){
                    ...
                    //int add_move = move + (int)strlen((*tar) -> name) + 1;

                    // recursively write each subdirectory
                    if (write_entries(fd, &((*tar) -> next), head, 1, (const char **) &path, offset, verbosity, move) < 0){
            ...
            // ((*tar) -> type == SYMLINK) || ((*tar) -> type == CHAR) || ((*tar) -> type == BLOCK) ||
            // ((*tar) -> type == FIFO)){
            V_PRINT(stdout, "Writing %s", (*tar) -> name);
            ...
            if (((*tar) -> type == REGULAR) || ((*tar) -> type == NORMAL) || ((*tar) -> type == CONTIGUOUS)){
                ...
                if (!tarred){
                    ...
                    int f = open((*tar) -> original_name, O_RDONLY);
                    if (f < 0){
